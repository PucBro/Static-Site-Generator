...
def generate_page(from_path:str,template_path:str, dest_path:str, base_path:str):
    ...
    template_contents = template_contents.replace("{{ Title }}", title).replace("{{ Content }}", content)
    final_html = template_contents.replace('href="/', f'href="{base_path}').replace('src="/', f'src="{base_path}')
...
def generate_pages_recursive(dir_path_content:str, template_path:str, dest_dir_path:str, base_path:str):
    ...
    for element in directories:
        ...
        if os.path.isfile(element_path):
            ...
            generate_page(element_path, template_path=template_path, dest_path=destiny_path, base_path=base_path)
        else:
            os.mkdir(destiny_path)
            generate_pages_recursive(dir_path_content=element_path, template_path=template_path, dest_dir_path=destiny_path, base_path=base_path)
